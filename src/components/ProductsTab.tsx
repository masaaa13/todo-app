...
function downloadMdProductsCsv(
  ...
  const rows = products.map((p) => {
    const cols = [
      ...
      String(stockValue(p)),
      String(numValue(p.recentSales)),
      p.sellThroughRate != null ? String(p.sellThroughRate) : '準備中',
      p.status,
      p.nextAction,
      p.productUrl ?? '',
      String(numValue(p.salesQty30d)),
      String(numValue(p.salesAmount30d)),
      String(numValue(p.monthlySalesQty)),
      String(numValue(p.monthlySalesAmount)),
...
function downloadMdVariationsCsv(
  ...
  const rows = variations.map((v) => {
    const cols = [
      ...
      v.releaseDate ?? '',
      String(stockValue(v)),
      String(numValue(v.recentSales)),
      v.sellThroughRate != null ? String(v.sellThroughRate) : '準備中',
      v.status,
      v.nextAction,
      v.productUrl ?? '',
      String(numValue(v.salesQty30d)),
      String(numValue(v.salesAmount30d)),
      String(numValue(v.monthlySalesQty)),
      String(numValue(v.monthlySalesAmount)),
