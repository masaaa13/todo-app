...
export function WishlistTab({ products }: Props) {
  ...
  const items = hasData ? toWishlistItems(products) : [];

  const highCount = items.filter((i) => i.priority === '高').length;
  const midCount  = items.filter((i) => i.priority === '中').length;
  const lowCount  = items.filter((i) => i.priority === '低').length;

  const kpis = [
    { label: '候補商品数',    value: hasData ? String(items.length) : '—', active: hasData },
    ...
    <div className={styles.root}>
      ...
      <div className={styles.pageHeader}>
        ...
        <span
          ...
          data-variant={hasData ? 'active' : 'empty'}
        >
      ...
      <div className={styles.mainLayout}>
        ...
        <div className={styles.conditionPanel}>
          <div className={styles.panelTitle}>現在の仮条件</div>
          <ul className={styles.conditionList}>
            <li>商品一覧に保存された商品を候補に表示</li>
            <li>在庫・売上データは未連携</li>
            <li>優先度はカテゴリ/補完状況をもとに仮判定</li>
            <li>店舗共有用CSVは仮出力</li>
          </ul>

          <div className={styles.conditionDivider} />

          <div className={styles.futureLabel}>将来的な条件</div>
          <ul className={styles.futureList}>
            {[
              ...
              <li key={c} className={styles.futureItem}>
                  ...
                  {items.map((item) => (
                    <tr key={item.productNo} className={styles.tr}>
                      ...
                      <td className={styles.td}>
                        <span className={styles.productNo}>{item.productNo}</span>
                      </td>
                      <td className={styles.td}>
                      ...
                      <td className={styles.td}>
                        <span className={styles.dateCell}>{item.releaseDate ?? '—'}</span>
                      </td>
                      <td className={styles.td}>
                        <span className={styles.numCell}>{item.skuCount ?? '—'}</span>
                      </td>
                      <td className={styles.td}>
                        <span className={styles.reasonCell}>{item.reason}</span>
                      </td>
                      <td className={styles.td}>
                        <span className={styles.actionCell}>{item.suggestedAction}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
